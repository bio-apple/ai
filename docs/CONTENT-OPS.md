...
| 工作流                                                                                                     | Cron（北京）           | 说明                                                                       |
| ---------------------------------------------------------------------------------------------------------- | ---------------------- | -------------------------------------------------------------------------- |
| [daily-refresh.yml](https://github.com/bio-apple/ai/actions/workflows/daily-refresh.yml)                   | 每日 **00:00**         | **串行**刷新：视频 → 开源 → 课程 → 排行 → 一次推送/部署 → 死链（不含新闻） |
| [daily-news.yml](https://github.com/bio-apple/ai/actions/workflows/daily-news.yml)                         | 每日 **07:30**         | 新闻热点单独刷新并派发 Deploy                                              |
| [site-health.yml](https://github.com/bio-apple/ai/actions/workflows/site-health.yml)                       | 每日 **08:00 / 20:00** | 线上 JSON 新鲜度探针（不改数据）                                           |
| `daily-videos.yml` / `daily-oss.yml` / `daily-courses.yml` / `daily-rankings.yml` / `daily-link-check.yml` | 无 cron（仅手动）      | 单频道救急重跑                                                             |
...
| 角色            | 职责                                                                                    | 落点                                       |
| --------------- | --------------------------------------------------------------------------------------- | ------------------------------------------ |
| **数据 / 后端** | 00:00 串行四频道 + 07:30 新闻；单频道失败记录但不中断其它频道；频道失败 → job hard-fail | `daily-refresh.yml` / `daily-news.yml`     |
...
| **QA / 探针**   | 北京 08:00 / 20:00 查线上 videos/news/courses/oss 新鲜度 ≤2 天                          | `site-health.yml` + `check_site_health.py` |
...
| **频率**     | 每日 00:00 串行队列（`daily-refresh.yml` 第 3 步）；可手动单跑 |
...
| **产出**     | `ai-news.json`                                 |
| **频率**     | 每日 **07:30**（`daily-news.yml`）；可手动单跑 |
...
| **site-health.yml**         | 探针检查视频/新闻/**课程/开源** JSON 是否在 2 天内更新                     |
| **daily-refresh.yml**       | 00:00 串行日更（不含新闻）+ lychee（软）；**频道失败**开 `[ops]` Issue     |
| **daily-news.yml**          | 07:30 新闻热点日更；失败开 `[ops] Daily news fetch failed`                 |
