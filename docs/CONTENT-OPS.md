...
flowchart TB
  ...
  subgraph auto["自动抓取（定时 / 手动脚本）"]
    ...
    A4["oss-projects.json — 日更"]
    A5["data/rankings.json — 日更"]
  end
...
| **开源精选** | [daily-oss.yml](https://github.com/bio-apple/ai/actions/workflows/daily-oss.yml)               | 每日 **07:00**         | `fetch_oss_stars.py`     | `data/oss-projects.json` + `oss-projects.json` |
| **课程资源** | [daily-courses.yml](https://github.com/bio-apple/ai/actions/workflows/daily-courses.yml)       | 每日 **08:00**         | `fetch_ai_courses.py`    | `ai-courses.json`                              |
| **工具排行** | [daily-rankings.yml](https://github.com/bio-apple/ai/actions/workflows/daily-rankings.yml)     | 每日 **09:00**         | `fetch_rankings.py`      | `data/rankings.json`                           |
| **死链检测** | [daily-link-check.yml](https://github.com/bio-apple/ai/actions/workflows/daily-link-check.yml) | 每日 **10:00**         | lychee（`.lychee.toml`） | 扫描外链；失败开/更新 Issue，不改数据          |
...
> 动态频道（视频 / 新闻 / 开源 / 课程 / 排行）**统一日更**。  
> **搜索索引**在每次 `npm run build`（prebuild）由 `build-artifacts.mjs` 从工具/资讯/开源/课程/视频等重新生成，无需单独抓取。
...
| **daily-link-check.yml**    | 每日 lychee 扫描外链；失效时开/更新 `[ops] Dead link check failed` Issue   |
| **失败 Issue**              | 各定时工作流失败时自动开/更新 `[ops]` Issue                                |
| **内容漏斗**                | 前端 `funnel.js` 埋点；分析维度见 [CONTENT-FUNNEL.md](./CONTENT-FUNNEL.md) |
...
- [ ] 「开源精选」「课程资源」「排行榜」`updated_at` 在 2 天内
- [ ] 「课程资源」五条路线均有课（每路线 ≤5）
- [ ] 「开源精选」Star 数合理、无重复仓库
- [ ] 无未关闭的 `[ops]` Issue（含 Dead Link / 抓取失败）
- [ ] [daily-link-check](https://github.com/bio-apple/ai/actions/workflows/daily-link-check.yml) 无未处理失败
- [ ] 本地或 CI 构建后搜索可用（顶栏联想 / 结果分组正常）
...
- [FRONTEND.md](./FRONTEND.md) — 搜索 / 快筛 / 虚拟列表等前端能力
- [CONTENT-FUNNEL.md](./CONTENT-FUNNEL.md) — 内容漏斗与分析事件
