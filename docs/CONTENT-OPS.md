...
| **死链检测** | [weekly-link-check.yml](https://github.com/bio-apple/ai/actions/workflows/weekly-link-check.yml) | 每周一（见 workflow） | lychee（`.lychee.toml`） | 扫描外链；失败开/更新 Issue，不改数据        |
...
| **weekly-link-check.yml**   | 每周 lychee 扫描外链；失效时开/更新 `[ops] Dead link check failed` Issue |
...
- [ ] [weekly-link-check](https://github.com/bio-apple/ai/actions/workflows/weekly-link-check.yml) 无未处理失败
