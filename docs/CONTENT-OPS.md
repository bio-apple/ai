...
| 角色            | 职责                                                                                  | 落点                                       |
| --------------- | ------------------------------------------------------------------------------------- | ------------------------------------------ |
| **数据 / 后端** | 00:00 串行四频道 + 新闻多档（07:30/10:00/12:00/20:00）；频道失败 → job hard-fail      | `daily-refresh.yml` / `daily-news.yml`     |
...
| AI 领域地图             | `hero-ai-map.svg` / `hero-ai-map-*.webp`             | 首页 `#home-ai-map` 内容区块；见 FRONTEND.md |
...
- [ ] 首页专区与工具中心可见「首页 / …」面包屑
- [ ] 首页 `#home-ai-map` 领域地图清晰可读，Hero 无信息图叠字
