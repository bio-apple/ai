...
| 工作流                                                                                                                        | Cron（北京）           | 说明                                                                  |
| ----------------------------------------------------------------------------------------------------------------------------- | ---------------------- | --------------------------------------------------------------------- |
| [daily-refresh.yml](https://github.com/bio-apple/ai/actions/workflows/daily-refresh.yml)                                      | 每日 **00:00**         | **串行**刷新：视频 → 新闻 → 开源 → 课程 → 排行 → 一次推送/部署 → 死链 |
| [site-health.yml](https://github.com/bio-apple/ai/actions/workflows/site-health.yml)                                          | 每日 **08:00 / 20:00** | 线上 JSON 新鲜度探针（不改数据）                                      |
| `daily-videos.yml` / `daily-news.yml` / `daily-oss.yml` / `daily-courses.yml` / `daily-rankings.yml` / `daily-link-check.yml` | 无 cron（仅手动）      | 单频道救急重跑                                                        |

> 所有动态频道在 **同一时刻（00:00）启动编排**，但**不会并行抓取**：上一频道抓取并本地 commit 完成后，才开始下一频道。  
...
| 角色           | 职责                                                                 | 落点                                                                 |
| -------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------- |
| **数据 / 后端** | 串行抓取五频道；单频道失败记录但不中断其它频道；频道失败 → job hard-fail | `daily-refresh.yml` fetch/commit 步骤                                |
...
| **频率**     | 每日 00:00 串行队列（`daily-refresh.yml`）；可手动单跑 |
...
| **daily-refresh.yml**       | 00:00 串行日更 + 末步 lychee（软告警）；**频道失败**开 `[ops]` Issue |
