...
| 交互层 | 原生 JS + Fuse.js      | 频道 Tab、搜索、推荐、知识库助手（客户端检索）          |
...
| **运行时** | `search-index.json`                    | `app.js`、`knowledge.js`             | Fuse.js 客户端全文检索                           |
...
数据有变更时，工作流会 `workflow_dispatch` 触发 `deploy.yml` 重新部署。
...
Schema 文件位于 `schemas/`；手工维护的 `site.json` / `tools.json` 目前校验 **JSON 可解析 + 交叉引用**（如 `tool-relations` 的 id 必须存在于 `tools.json`）。
...
src/components/       # 可复用 UI 块
src/layouts/          # 页面壳
src/lib/              # data 加载、路径、Schema.org
