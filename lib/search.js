...
(function initSearchLib() {
  ...
  function siteBase() {
    if (typeof window.BioAI.siteBase === 'function') return window.BioAI.siteBase();
    const raw = document.documentElement.dataset.base || '/ai/';
    return raw.endsWith('/') ? raw : `${raw}/`;
  }

  function sectionPageMap() {
    return window.BioAI.SECTION_PAGE_MAP || {};
  }
  ...
  function gotoSearchHit(item, query = '') {
    ...
    if (item.section) {
      const page = sectionPageMap()[item.section];
      if (page) {
      ...
      window.showSection?.(item.section, { anchor: item.anchor || null });
  ...
  function renderSuggestionsPanel(wrap, input, results) {
    ...
    html += renderAskKnowledgeRow('');
    results.innerHTML = html;
    setSearchDropdownOpen(wrap, input, results, true);
    bindKnowledgeActions(wrap, input, results);
  ...
  function requestKnowledge(query, { ask = false } = {}) {
    const q = String(query || '').trim();
    const go = () => {
      if (typeof window.BioAI?.openKnowledge === 'function') {
        window.BioAI.openKnowledge({ query: q, ask: Boolean(ask && q) });
        return true;
      }
      return false;
    };
    if (go()) return;
    const ensure = window.__BIOAI_ensureKnowledge;
    if (typeof ensure !== 'function') return;
    Promise.resolve(ensure()).then(() => {
      const start = Date.now();
      const wait = () => {
        if (go() || Date.now() - start > 4000) return;
        setTimeout(wait, 40);
      };
      wait();
    });
  ...
  function renderAskKnowledgeRow(query) {
    const q = String(query || '').trim();
    const shown = q.length > 32 ? `${q.slice(0, 32)}…` : q;
    const label = q ? `用站内索引搜索「${escapeHtml(shown)}」` : '在本站搜索';
    const action = q ? 'ask-knowledge' : 'open-knowledge';
    const hitClass = q ? 'search-hit search-hit-ask' : 'search-ask';
    return `<button type="button" class="${hitClass}" role="option" data-action="${action}" data-query="${escapeHtml(q)}">${label}<span class="search-hit-meta">检索</span></button>`;
  }

  function bindKnowledgeActions(wrap, input, results) {
    results
      .querySelectorAll('[data-action="ask-knowledge"], [data-action="open-knowledge"]')
      .forEach((btn) => {
        ...
        btn.addEventListener('click', (e) => {
          ...
          requestKnowledge(q, { ask: btn.dataset.action === 'ask-knowledge' && !!q });
        });
      });
  }
  ...
  function renderSearchResults(wrap, input, results, rawQuery) {
    ...
    if (!hits.length) {
      ...
      <div class="search-empty">
        ...
        ${renderAskKnowledgeRow(query)}
        <div class="search-empty-actions">
          <a href="#home-recommend" class="search-empty-link" data-track="search_empty_recommend">按任务匹配工具</a>
    ...
    results.innerHTML = [...grouped.entries()]
      ...
      .join('');
    results.innerHTML += renderAskKnowledgeRow(query);
    setSearchDropdownOpen(wrap, input, results, true);
  ...
  function bindSearchHotkey() {
    if (document.documentElement.dataset.searchHotkey === '1') return;
    document.documentElement.dataset.searchHotkey = '1';
    document.addEventListener('keydown', (e) => {
      ...
      if (!input) return;
      e.preventDefault();
      window.__BIOAI_ensureFuse?.();
      window.__BIOAI_ensureKnowledge?.();
      window.BioAI?.closeKnowledge?.();
      input.focus();
      input.select();
    });
  }
  ...
  function initSearch() {
    paintSearchKbd();
    bindSearchHotkey();
    return loadSearchIndex().finally(() => {
  ...
  window.BioAI.initSearch = initSearch;
})();
