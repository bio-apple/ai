...
(function initSearchLib() {
  ...
  function renderAskKnowledgeRow(query) {
    ...
    const label = q ? `用知识库回答「${escapeHtml(shown)}」` : '打开知识库助手';
    const action = q ? 'ask-knowledge' : 'open-knowledge';
    const hitClass = q ? 'search-hit search-hit-ask' : 'search-ask';
    return `<button type="button" class="${hitClass}" role="option" data-action="${action}" data-query="${escapeHtml(q)}">${label}<span class="search-hit-meta">对话</span></button>`;
  }

  function bindKnowledgeActions(wrap, input, results) {
    results.querySelectorAll('[data-action="ask-knowledge"], [data-action="open-knowledge"]').forEach((btn) => {
