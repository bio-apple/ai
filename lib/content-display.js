...
export function ossAudienceTags(item) {
  const defaults =
    OSS_AUDIENCE[item?.category || ''] || [item?.categoryLabel || '开源'].filter(Boolean);
  const categoryDupes = new Set([item?.categoryLabel, defaults[0]].filter(Boolean));
  const tags = [...defaults];
  if (item?.isNew) tags.push('新手可盯');
  if ((item?.stars || 0) >= 50_000) tags.push('社区主流');
  return [...new Set(tags.filter((tag) => tag && !categoryDupes.has(tag)))];
}
...
export function rankingPickReason(name, _boardId = '') {
