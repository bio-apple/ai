...
(function initLinkGuard() {
  ...
  function showDeadLinkDialog(url, reason) {
    ...
    dlg.setAttribute('aria-labelledby', 'bio-deadlink-title');
    dlg.setAttribute('aria-describedby', 'bio-deadlink-desc');
    dlg.innerHTML = `
      <div class="deadlink-dialog-card">
        <h3 id="bio-deadlink-title">链接可能已失效</h3>
        <p id="bio-deadlink-desc">${escapeHtml(reason || '目标页面暂时无法访问（仓库下架、视频删除或网络限制）。')}</p>
    ...
    dlg.querySelector('[data-action="close"]')?.focus();
