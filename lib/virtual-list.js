        ...
        <div class="daily-vl-row">
          <a href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer"
            ...
            <span class="daily-item-meta">
              <span class="news-source-chip news-source-chip--${escapeHtml(item.sourceKey || 'github')}">
                ${item.sourceLogo ? `<img class="news-source-logo" src="${escapeHtml(item.sourceLogo)}" alt="" width="18" height="18" loading="lazy" decoding="async">` : ''}
                <span class="news-source-mark" aria-hidden="true">${escapeHtml(item.sourceMark || 'GH')}</span>
                <span class="news-source-label">${escapeHtml(item.meta || 'GitHub')}</span>
              </span>
              ${item.relative ? `<time class="daily-item-date" datetime="${escapeHtml(item.published_at || '')}">${escapeHtml(item.relative)}</time>` : ''}
            </span>
          </a>
        </div>
      `,
