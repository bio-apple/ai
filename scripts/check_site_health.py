...
def remediation_for(exc: BaseException) -> list[str]:
    ...
    if msg.startswith("HTTP:") or "URLError" in type(exc).__name__:
        ...
        lines += [
            ...
            "2. 对照 `docs/CONTENT-OPS.md` §4 告警分级表",
...
def main() -> int:
    ...
    try:
        ...
        check_http("/oss.html", expect_substr="开源精选")
        notes.append("- oss OK")
        check_http("/courses.html", expect_substr="课程")
        notes.append("- courses OK")
        check_http("/news/daily-ai-news.html", expect_substr="AI 新闻热点")
        notes.append("- news OK")
        check_http("/videos.html", expect_substr="AI 视频")
        notes.append("- videos OK")
