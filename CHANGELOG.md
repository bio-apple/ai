...
- 首页匹配是关键词规则（最多 3 个工具 + 一步），不是 LLM 助手；简报固定资讯 / 升温 / 编辑视频
- 日更视频：关键词 + 拒绝表后再按播放量取每平台 Top 3；首页片单 `data/home-video-picks.json`
- 三榜过期写「数据停在这一天（未日更）」；`pick_reason` 只展示人工对照表，未写则空
- 独立页没有可见 `h2–h4` 时不画空目录侧栏
- 场景点击 / 未匹配 / 手删视频写入 `localStorage` 键 `bioai.flywheel`（不接 LLM）

## [3.0.0] — 2026-09-08

站名 **AI 导航**。顶栏：首页、AI 工具中心、开源精选、课程资源、新闻热点、AI 视频。

- 首页：按任务匹配、AI 简报、知识版图（绿圈可点，黄圈只作图示）、下一步
- 新闻热点：滚动 7×24h；量子位官网热门近 30 天按既有分类并入
- AI 视频：近 1 个月 YouTube / B 站过滤后各 Top 3；粘贴收藏 + Cloudflare 同步
