...
- 知识版图加回基础学科黄圈；图下方按钮不含「基础学科」一组；黄圈只作图示、不再跳转
- 下线「选型指南」对比专题（一句话结论 / 功能矩阵 / 参数对比 / 用户评价整页去掉）
- 下线「AI 学习路线」页：阶段清单过时、不再维护
- 下线「零基础 AI 入门指南」：内容与课程页重叠，入口改为课程资源
- 下线首页「常见问题」：对比页已撤，问答入口不再保留
- 下线「我的收藏」页：仅本机存储、入口多余
- 文档与 TDK 按产品/前端/运维对齐：不再把已下线的对比专题当主入口
